...
@Component({
  selector: 'app-assigned-bug-mainlist',
  ...
  templateUrl: './assigned-bug-mainlist.component.html',
  styleUrl: './assigned-bug-mainlist.component.css'
})
export class AssignedBugMainlistComponent {
displayedColumns: string[] = ['id', 'projectid', 'title', 'description', 'priority', 'status', 'created', 'updated', 'reportedBy', 'assignedTo'];
  ...
  statusOptions: string[] = ['In Progress','Resolved', 'Open'];
  bugs: bug[] = [];
  ...
  BugPage(bugid: number, projectid: number) {
    ...
    this.router.navigate([`/bugs/developer/user/`+ this.userid+ `/project/`+ projectid+ `/bug/`, bugid]);
}
